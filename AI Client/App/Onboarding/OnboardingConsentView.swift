...
- API 密钥（API Keys）：本应用采用“自备密钥（BYOK）”模式。您在应用内输入的任何 API 密钥（包括但不限于 DeepSeek、OpenRouter、OpenAI 等供应商的 Key）均存储在您本地设备的 iOS Keychain 中。
...
对话和知识库等应用容器数据可通过应用内清除功能或卸载应用删除。API 密钥存储于 iOS Keychain；如需明确删除，请先在应用内删除对应的 Provider 配置或 API 密钥。删除本地数据不会删除已发送至第三方供应商的数据。
